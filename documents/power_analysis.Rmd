...
```{r param-table}
df_param <- tibble('Parameter' = c("$\\phi_1$", "$\\phi_2$", "$\\phi_3$", "$p_2$", "$p_3$", "$p_4$"),
                   'True Value' = c(phi1, phi2, phi3, p2, p3, p4))
...
subtitle_text <- "$\\text{Simulated survival estimates of juveniles to IPTDS and Lower Granite Dam;} \\, \\phi_1 = "
subtitle_text <- str_wrap(paste0(subtitle_text, phi1, ", \\phi_2 = ", phi2, ", \\rho_2 = ", p2, ", \\text{ and } \\rho_3 = ", p3, "$"),60)
...
```{r det-caption}
subtitle_text <- as.character(bquote("Simulated detection estimates of juveniles at IPTDS and Lower Granite Dam; " ~ 
                          rho[2] == .(p2) ~ ", and " ~ 
                          rho[3] == .(p3)))
subtitle_text <- "$\\text{Simulated detection estimates of juveniles at IPTDS and Lower Granite Dam;} \\, \\rho_2 = "
subtitle_text <- str_wrap(paste0(subtitle_text, p2, ", \\rho_3 = ", p3, "$"),60)
```
...
```{r precision, fig.cap = 'Coefficient of variation around survival estimates.'}
## precision
df_sum %>%
  filter(param == 'Phi',
         time %in% c(1,2)) %>%
